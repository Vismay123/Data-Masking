...
if (isset($_POST["export"])) {
     ...
     function maskData($data) {
          // Only mask if length is greater than 2
          if (strlen($data) > 2) {
              $firstTwo = substr($data, 0, 2);
              $masked = str_repeat('#', strlen($data) - 2);
     ...
     if (mysqli_num_rows($result_form_1) > 0) {
      ...
      while ($row_form_1 = mysqli_fetch_array($result_form_1)) {
          // Mask the required fields
          $maskedPatientName   = maskData($row_form_1["patient_name"]);
          $maskedPatientName1  = maskData($row_form_1["patient_name1"]);
          $maskedClinicRegNo   = maskData($row_form_1["clinic_regno"]);
          $maskedAddress       = maskData($row_form_1["address"]);
          $maskedEmail         = maskData($row_form_1["email_id"]);
          $maskedAge           = maskData($row_form_1["Age"]);
          $maskedSex           = maskData($row_form_1["sex"]);
          
          $output .= '
          <tr>  
              <td>' . $row_form_1["pid"] . '</td>  
              <td>' . $maskedPatientName . '</td>  
              <td>' . $row_form_1["study_no"] . '</td>  
              <td>' . $row_form_1["date"] . '</td>  
              <td>' . $row_form_1["staff_name"] . '</td>  
              <td>' . $maskedPatientName1 . '</td>  
              <td>' . $maskedClinicRegNo . '</td>  
              <td>' . $maskedAddress . '</td>  
              <td>' . $maskedEmail . '</td>  
  ...
  // Third table - contact numbers (masking applied)
  if (mysqli_num_rows($result_form_1_contact) > 0) {
      ...
      while ($row_form_1_contact = mysqli_fetch_array($result_form_1_contact)) {
          ...
          $maskedContact = maskData($row_form_1_contact["contact_no"]);
          $output .= '
              <tr>  
